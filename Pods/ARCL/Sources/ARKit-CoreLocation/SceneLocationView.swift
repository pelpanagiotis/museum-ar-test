...
public extension SceneLocationView {
    ...
    func addRoutes(polylines: [AttributedType<MKPolyline>],
        ...
        let coords1 = [CLLocationCoordinate2D(latitude: 40.661304, longitude: 22.954058), CLLocationCoordinate2D(latitude: 40.661543, longitude: 22.953979)]
        let polyline1 = MKPolyline(coordinates: coords1, count: coords1.count)
        
        let customPolyNode = PolylineNode(polyline: polyline1, altitude: altitude + Δaltitude)
        polylineNodes.append(customPolyNode)
        
//        customPolyNode.locationNodes.forEach {
...
//        sceneNode?.addChildNode(customPolyNode)
    }
